...
pthread_cond_t nextRed, nextBlue;
...
void * producer (void *arg)
{
	...
	for ( i = 0; i< 20; i++) {
		...
		// if (color == 0) { while( pthread_cond_wait( &nextRed, &lock ) != 0) ; }
		// else { while( pthread_cond_wait( &nextBlue, &lock ) != 0) ; }
...
int main( int argc, char* argv[] )
{
	...
	pthread_cond_init( &nextBlue, NULL);
	pthread_cond_init( &nextRed, NULL);
